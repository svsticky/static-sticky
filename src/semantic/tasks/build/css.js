  ...
  return gulp
    ...
    .pipe(
      replace(
        ...
        compress
          ? config.paths.assets.compressed
          : config.paths.assets.uncompressed
      )
    )
...
function pack(type, compress) {
  const output =
    type === 'docs' ? docsConfig.paths.output : config.paths.output;
  const ignoredGlobs =
    type === 'rtl' ? globs.ignoredRTL + '.rtl.css' : globs.ignored + '.css';
...
function buildCSS(src, type, config, opts, callback) {
  if (!install.isSetup()) {
    console.error(
      'Cannot build CSS files. Run "gulp install" to set-up Semantic'
    );
    callback();
    return;
  }
